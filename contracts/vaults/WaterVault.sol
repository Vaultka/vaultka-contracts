 ...
 * @author Vodka
 **/
contract WaterVault is IWaterVault, ERC4626Upgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    //@note for uniswap router, refactoring needed to avoid hardcoded addresses
    ISwapRouter public swapRouter;

    address public constant USDC = 0xFF970A61A04b1cA14834A43f5dE4533eBDDB5CC8;
    address public constant USDT = 0xFd086bC7CD5C481DCC9C85ebE478A1C0b69FCbb9;
    address public constant DAI = 0xDA10009cBd5D07dd0CeCc66161FC93D7c9000da1;
    //@note should allow fee to to a vairable, hardcoding for simplicity
    ...
    modifier onlyBorrower() {
        if (msg.sender != address(VodkaVault) && msg.sender != address(leveragePool)) revert CallerNotBorrower();
        _;
    }
    ...
    /// @param _VodkaVault: updated deposit cap
    function setVodkaVault(IBorrower _VodkaVault) external onlyOwner {
        VodkaVault = _VodkaVault;
        emit VodkaVaultUpdated(_VodkaVault);
    }
    ...
    function updateBorrowCap(address borrowerAddress, uint256 cap) external onlyOwner {
        if (borrowerAddress != address(VodkaVault) && borrowerAddress != address(leveragePool))
    ...
    /// @notice deposit usdc, usdt or dai
    ...
    function userDeposit(
        ...
        require(token == USDC || token == USDT || token == DAI, 'unsupported token');

        if (token == USDT || token == DAI) {
            amount = _swapStableForUSDC(token, amount);
        }
    ...
    // @note is support for stable coins applicable to mint as well?
    ...
    function userWithdraw(
        ...
        require(token == USDC || token == USDT || token == DAI, 'unsupported token');
        if (token == USDC) {
            withdraw(assets, to, to);
        } else {
            withdraw(assets, address(this), to);
            _swapUSDCForStable(token, assets, to);
        }
    }
    ...
    function _swapStableForUSDC(address token, uint256 amountIn) internal returns (uint256 amountOut) {
        //@note unlike junior vault, we need to first transfer money into this contract first
    ...
    function _swapUSDCForStable(
        address token,
        uint256 amountIn,
        address receiver
    ) internal returns (uint256 amountOut) {
        //@note doesn't need this since we already got the money in the contract
        // TransferHelper.safeTransferFrom(token, msg.sender, address(this), amountIn);
    ...
    function totalUsdcBorrowed() public view returns (uint256 usdcBorrowed) {
        ...
        if (address(VodkaVault) != address(0)) usdcBorrowed += VodkaVault.getUsdcBorrowed();
    }
