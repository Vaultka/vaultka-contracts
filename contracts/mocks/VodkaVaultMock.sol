...
import { VodkaVault } from '../vaults/VodkaVault.sol';

contract VodkaVaultMock is VodkaVault {
    ...
    function executeBorrowFromWaterVault(uint256 amount) external {
        state.WaterVault.borrow(amount);
    }

    function executeRepayFromWaterVault(uint256 amount) external {
        state.WaterVault.repay(amount);
    }
