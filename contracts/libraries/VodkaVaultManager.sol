...
/**
 * is junior vault delegating the logic to this contract rn?
...
library VodkaVaultManager {
    event RewardsHarvested(
        ...
        uint256 esGmxStaked,
        uint256 vodkaVaultWeth,
        uint256 waterVaultWeth,
        uint256 vodkaVaultGlp,
        uint256 waterVaultAUsdc
    );
    ...
    struct State {
        ...
        uint16 withdrawFeeBps; // 2 bytes
        // fee tier for uniswap path
        uint24 feeTierWethWbtcPool; // 4 bytes
    ...
    function harvestFees(State storage state) public {
        ...
        uint256 wethHarvested = state.weth.balanceOf(address(this)) - state.protocolFee - state.waterVaultWethRewards;
        ...
        if (wethHarvested > state.wethConversionThreshold) {
            ...
            uint256 _waterVaultWethRewards = state.waterVaultWethRewards + WaterVaultWethShare;
            ...
            if (_waterVaultWethRewards > state.wethConversionThreshold) {
                ...
                uint256 minUsdcAmount = _getTokenPriceInUsdc(state, state.weth).mulDivDown(
                    _waterVaultWethRewards * (MAX_BPS - state.slippageThresholdSwapEthBps),
                    MAX_BPS * PRICE_PRECISION
                );
                // swaps weth into usdc
                (uint256 aaveUsdcAmount, ) = state._swapToken(
                    address(state.weth),
                    _waterVaultWethRewards,
                    minUsdcAmount
                );
                ...
                state.waterVaultWethRewards = _waterVaultWethRewards;
                emit RewardsHarvested(
                    wethHarvested,
                    sGmxHarvested,
                    dnGmxWethShare,
